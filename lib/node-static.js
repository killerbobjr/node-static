...
var Server = function(root, options)
{
	...
	if(root === undefined)
	{
		this.root.push(path.normalize(path.resolve('.')));
	...
	this.options.rewrite = options.rewrite || [{regex:'(.*)',format:'$1'}];
...
Server.prototype.serveFile = function(urlpath, status, headers, req, res)
{
	var	that = this,
	...
	for(var root of this.root)
	{
		for(var rewrite of that.options.rewrite)
		{
			pathname = this.resolve(root, urlpath.replace(new RegExp(rewrite.regex), rewrite.format));
...
Server.prototype.servePath = function(urlpath, status, headers, req, res, finish)
{
	var that = this,
		promise = new(events.EventEmitter),
		stat,
		...
		for(var rewrite of that.options.rewrite)
		{
			...
			if(pathname.indexOf(root) === 0)
			{
				...
				if(stat !== undefined)
				{
					...
					{
						err = new Error('Not a file or folder.');
					}
				}
			}
			else
...
Server.prototype.resolve = function(root, pathname)
{
	return path.resolve(path.join(root, pathname));
};
...
Server.prototype.serve = function(req, res, callback)
{
	...
	process.nextTick(function()
		{
			that.servePath(pathname, 200, {}, req, res, finish)
...
Server.prototype.gzipOk = function(req, contentType)
{
	...
	if(enable &&
		(typeof enable === 'boolean' || (contentType && (enable instanceof RegExp) && enable.test(contentType))))
	{
